  ...
  return (
    <div className="absolute inset-0 flex flex-col justify-between p-5 sm:p-7">
      <div className="flex justify-end">
        <div className="inline-flex items-center gap-1.5 bg-white/10 sm:backdrop-blur-md rounded-full px-3 py-1.5 border border-white/10">
          <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
          <span className="text-xs font-semibold text-white">{house.rating}</span>
          <span className="text-white/50 text-xs">({house.reviewCount})</span>
        </div>
      </div>
      <div>
...
function MobileCard({ house, index }: { house: HouseData; index: number }) {
  return (
    <motion.div
      ...
      transition={{ duration: 0.6, delay: 0.2 + index * 0.12, ease: [0.21, 0.47, 0.32, 0.98] }}
    >
      <div className="relative rounded-2xl overflow-hidden ring-1 ring-black/5 dark:ring-white/5">
        <Link
...
/* ── Desktop card: full 3D tilt, springs, glare ── */
function DesktopCard({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.7, delay: 0.3 + index * 0.15, ease: [0.21, 0.47, 0.32, 0.98] }}
    >
      <motion.div
        ref={cardRef}
        ...
        animate={{
          ...
          filter: isSiblingHovered ? "brightness(0.5)" : "brightness(1)",
        }}
        transition={{ duration: 0.5, ease: [0.25, 0.46, 0.45, 0.94] }}
        className="relative rounded-2xl overflow-hidden ring-1 ring-black/5 dark:ring-white/5"
      >
        <Link
          href={`/house/${house.slug}`}
    ...
    <div className="relative min-h-dvh w-full bg-[#fafafa] dark:bg-[#0a0a0a] overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100dvh-5rem)] px-6 sm:px-10 lg:px-16">
        <div className="text-center mb-10 sm:mb-14">
          ...
          <motion.h1
            className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-semibold text-gray-900 dark:text-white tracking-tight leading-[1.1]"
